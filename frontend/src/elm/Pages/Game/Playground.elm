module Pages.Game.Playground exposing
    ( game
    ...
    , Number
    , Game, initGame, subscriptionsGame, updateGame, viewGame, Msg(..)
    --
    --
    --
    --
...
{-| COPIED FROM <https://github.com/evancz/elm-playground/blob/1.0.3/src/Playground.elm>
...
game viewMemory updateMemory initialMemory =
    let
        init () =
            initGame initialMemory

        view model =
            { title = "Playground"
            , body = [ viewGame viewMemory model ]
            }
    in
    Browser.document
        { init = init
...
initGame : memory -> ( Game memory, Cmd Msg )
initGame initialMemory =
...
viewGame : (Computer -> memory -> List Shape) -> Game memory -> Html.Html Msg
viewGame viewMemory (Game _ memory computer) =
    render computer (viewMemory computer memory)
...
initialComputer =
  ...
  in
    ...
    , positionX = screen.left
    , positionY = screen.bottom
    }
...
gameUpdate updateMemory msg (Game vis memory computer) =
    case msg of
        ...
        GotViewport { viewport } ->
          ...
          in
          Game vis memory { computer | screen = screen, positionX = screen.left, positionY = screen.bottom }
        ...
        MouseWheel deltaY ->
          Game vis memory { computer | positionY = computer.positionY + deltaY }
...
render : Computer -> List Shape -> Html.Html Msg
render computer shapes =
    let
        w =
            String.fromFloat computer.screen.width

        h =
            String.fromFloat computer.screen.height

        x =
            String.fromFloat computer.positionX

        y =
            String.fromFloat computer.positionY
    in
    svg
        [ viewBox (x ++ " " ++ y ++ " " ++ w ++ " " ++ h)
        ...
        , width "100%"
        , height "100%"
        , Html.Events.on "wheel" (D.map MouseWheel (D.field "deltaY" D.float))
        ]
