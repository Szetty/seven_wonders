...
- Produces: a repository with no `frontend/` directory and no references to it outside `docs/` and `MIGRATION.md`.
...
Expected: shows ` M frontend/src/elm/Pages/Login.elm` plus untracked/ignored leftovers.

- [ ] **Step 2: STOP — ask the human for confirmation**

Do not run any deletion command before the human replies. Ask exactly:

> Phase 4 is ready to delete `frontend/`. Heads-up: you have an **uncommitted edit in `frontend/src/elm/Pages/Login.elm`** (2 changed lines). Deleting `frontend/` will **permanently lose it**, along with the untracked/ignored files `frontend/.DS_Store`, `frontend/public/.DS_Store` and `frontend/elm-stuff/`. Git refuses a plain `git rm -r frontend` while that file has local changes, so I would run `git rm -r -f frontend` and then `rm -rf frontend` for the leftovers.
> Reply with one of:
> 1. **"save patch then delete"**: I first write your edit to `~/seven_wonders-Login.elm.patch`, then delete.
> 2. **"delete"**: delete without saving.
> 3. **"stop"**: leave `frontend/` in place; I'll finish the remaining cleanup and report.
...
- [ ] **Step 3: (only after confirmation) Optionally save the edit, then delete**

For "save patch then delete":

```bash
git diff -- frontend/src/elm/Pages/Login.elm > ~/seven_wonders-Login.elm.patch
test -s ~/seven_wonders-Login.elm.patch && echo "patch saved"
```

Then, for either confirmation:

```bash
git rm -r frontend
```

This fails with "the following file has local modifications: frontend/src/elm/Pages/Login.elm". Since the human has confirmed, run:
...
Run: `git grep -nE "frontend(/|\b)" -- ':!docs' ':!MIGRATION.md'`
